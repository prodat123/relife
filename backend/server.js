...
app.get('/quests/completed', async (req, res) => {
    ...
    try {
        // Query to fetch completed quests for the given user where completed = true
        const [completedQuests] = await db.query(
            `SELECT qp.quest_id, qp.progress, qp.completed, qp.joined_at, q.name, q.description 
             FROM quest_participants qp
             INNER JOIN quests q ON qp.quest_id = q.id
...
app.post('/update-equipment', async (req, res) => {
    ...
    try {
        // Retrieve the current user's stats (from the `stats` column)
        ...
        let userStats = JSON.parse(userStatsResult[0].stats);
        ...
        if (itemId !== null) {
            ...
            // Handle the case where an item was previously equipped
            ...
            // Make sure stats are correctly updated
        ...
        res.status(200).json({ message: 'Equipment updated and stats modified successfully' });
    } catch (error) {
