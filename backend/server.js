...
app.get('/garden', async (req, res) => { 
    ...
    try {
        ...
        for (const plant of gardens) {
            ...
            if (plant.status === 'growing' && timeSinceLastWatered >= wiltTimeSec) {
                ...
                plant.status = 'wilted';
                plant.last_wilted_at = new Date(currentTime * 1000);
                continue;
            }
        }
...
app.get('/plant', async (req, res) => {
    ...
    try {
        ...
        const [gardens] = await connection.query(`
            SELECT pg.id, s.name, pg.planted_at, pg.last_watered_at, pg.status, 
            ...
            FOR UPDATE`, // 🔴 This locks the row for the transaction
            [userId, plantId]
        );

        if (gardens.length === 0) {
            await connection.rollback(); // Rollback changes if plant not found
        ...
        const nextWaterTime = lastWatered + plant.wilt_time;
        ...
        // Update total_progress and next_water_time safely within transaction
